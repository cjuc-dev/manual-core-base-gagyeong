...
## 🛠️ 주요 업데이트 사항 (Final Version)

1.  **All-in-One 서버 엔진**: 보안 및 실행 환경 최적화를 위해 공지사항 수집 엔진을 서버 내부로 통합하였습니다. 이제 `start.bat` 하나로 서버 가동과 데이터 갱신이 동시에 이루어집니다.
2.  **스마트 네비게이션**: 긴 문서에서도 편리하게 이동할 수 있는 가로형 스티키 네비게이션 바(홈, 맨 위로, 닫기)를 도입하였습니다.
3.  **데이터 계층화**: 시설 매뉴얼(`facilities`)과 시스템 데이터(`system`)를 분리하여 관리 효율성을 극대화하였습니다.
4.  **검색 고도화**: 검색어 하이라이트 및 결과 위치로의 자동 스크롤 기능을 완벽하게 지원합니다.
