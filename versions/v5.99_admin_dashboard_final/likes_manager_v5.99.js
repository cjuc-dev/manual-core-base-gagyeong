 ...
 * 
 * [기능 정의]
 * Smart Operations Suite (SOS) 매뉴얼에 대한 실시간 좋아요 햅틱 피드백 엔진 및 
...
window.getLikeData = async function(contentId) {
    ...
    let count = 0; // 기본 수치 (실운영 개시를 위해 0부터 시작)
    try {
        if (window.db) {
            ...
            if (docSnap.exists) {
                ...
                if (data[contentId] !== undefined) {
                    ...
                    // Firestore 상에 존재하지 않을 경우 기본값 0으로 즉시 서버 등재
                    await db.collection('manual').doc('likes').update({
                        [contentId]: 0
                    });
                }
            } else {
                // 문서 자체가 존재하지 않을 경우 안전한 병합 생성
        ...
        console.warn("[Firestore Likes] 실시간 좋아요 수 획득 실패 (기본값 작동):", e);
    }
...
/**
 * 🚀 [좋아요 실시간 토글 및 Firestore 원자적 증감 트랜잭션 집행]
 */
window.toggleLike = async function(contentId) {
    ...
    try {
        if (window.db) {
            // Firestore 원자적 필드 증감 연산(increment) 활용 (set merge를 활용해 문서 부재 시에도 안전 생성)
            await db.collection('manual').doc('likes').set({
                [contentId]: firebase.firestore.FieldValue.increment(incrementVal)
            }, { merge: true });

            // 🚀 [v5.93] 실시간 사용성 인텔리전스 분석 좋아요(Likes) 랭킹 컬렉션 동시 업데이트
            ...
            db.runTransaction(async (transaction) => {
                ...
                if (!sfDoc.exists) {
                    transaction.set(likeRef, {
                        contentId: contentId,
                        title: cleanTitle,
                        likeCount: Math.max(0, 0 + incrementVal),
                        lastLiked: new Date()
                    });
                } else {
            ...
            if (nextLikedState) {
                window.showToast("💖 소중한 좋아요가 전사 실시간 서버에 안전하게 기록되었습니다!");
            } else {
                window.showToast("💔 좋아요가 취소되었습니다.");
            }
        }
    } catch (e) {
