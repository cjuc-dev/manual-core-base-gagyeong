// --- CONTENT LOADING LOGIC WITH ERROR HANDLING ---
...
function loadContent(section, addHistory = true) {
    ...
    if (contentPaths[section]) {
        loadExternalContent(section, contentPaths[section]);
    } else {
...
function loadExternalContent(section, url) {
    const contentArea = document.getElementById('content-area');
    console.log(`Fetching URL: ${url}`);

    // 타임아웃 설정
    const timeoutId = setTimeout(() => {
        showError('요청 시간이 초과되었습니다.', section, url);
    }, LOAD_TIMEOUT);
    ...
    fetch(`${url}?t=${timestamp}`)
        ...
        .catch((error) => {
            ...
            showError(error.message, section, url);
        });
}

function showError(errorMessage, section, url) {
    const contentArea = document.getElementById('content-area');
    contentArea.innerHTML = `
        <div class="error-container">
            <i data-lucide="alert-circle" class="error-icon"></i>
            <h3 class="error-title">콘텐츠를 불러올 수 없습니다</h3>
            <p class="error-message">${errorMessage}</p>
            <button onclick="retryLoadContent('${section}', '${url}')" class="retry-button">
                <i data-lucide="refresh-cw" class="w-4 h-4 inline mr-2"></i>
                다시 시도
            </button>
            <button onclick="showDashboard()" class="retry-button" style="background: linear-gradient(135deg, #6366f1 0%, #4f46e5 100%); margin-left: 0.5rem;">
...
function retryLoadContent(section, url) {
    loadExternalContent(section, url);
}
...
async function toggleInlineExternalContent(containerId, url) {
    ...
    if (container.innerHTML.trim() === '') {
        try {
            ...
            // Re-execute scripts if necessary
